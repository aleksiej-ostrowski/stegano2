# original file https://github.com/sheerluck/andrew/blob/master/codilla.py
...
def encode(bch: galois.BCH, fn: str, fo: str) -> None:
    # 8 bytes - len of fn
    # 8 bytes - len of file
    # basename
    # content
...
def decode(bch: galois.BCH, fn: str, fo: str) -> None:
    ...
    print(f"ℹ️ new name is [red]{fo}[/red]")
    extract_content(bch, fo, fsize, tail)


def worker(input_file: str, output_file: str, mode: str) -> int:
    print(f"👉 [red]Creating BCH...[/red]")
    bch: Final[galois.BCH] = galois.BCH(127, 64)
    match mode:
        case "e" :
            encode(bch, input_file, output_file)
        case "d" :
            decode(bch, input_file, output_file)
        case _:  
            print("the mode must be e | d")
    print("All done! ✨ 🍰 ✨")
    return 0
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(        
        description="This script to encode and decode file with error correction code."        
    )        

    parser.add_argument(        
        "-input", "--input_file", help="Input file for processing", required=True, type=str    
    )        

    parser.add_argument(        
        "-output", "--output_file", help="Output file for processing", required=True, type=str       
    )     

    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("-e", "--encode", action='store_true', help='encode the file')
    group.add_argument("-d", "--decode", action='store_true', help='decode the file')

    args = parser.parse_args()    

    if args.encode:
        worker(args.input_file, args.output_file, "e")
    elif args.decode:
        worker(args.input_file, args.output_file, "d")
